...
import { Paintbrush, Eraser, Stamp, Sparkles, Palette, Download, RotateCcw } from "lucide-react"

interface ImprovedMSPaintProps {
  initialImage?: string | null
  width: number
  height: number
  onSave?: (imageData: string) => void
  canvasRef?: React.RefObject<HTMLCanvasElement>
  showEmojiStamper?: boolean
  emojiStamperEnabled?: boolean
}

export const ImprovedMSPaint: React.FC<ImprovedMSPaintProps> = ({
  initialImage = null,
  width,
  height,
  onSave,
  canvasRef: externalCanvasRef,
  showEmojiStamper = false,
  emojiStamperEnabled = false,
}) => {
  const internalCanvasRef = useRef<HTMLCanvasElement>(null)
  const canvasRef = externalCanvasRef || internalCanvasRef

  const [isDrawing, setIsDrawing] = useState(false)
  const [tool, setTool] = useState<"brush" | "eraser" | "stamp" | "sparkle" | "rainbow">("brush")
  ...
  const downloadCanvas = () => {
    ...
    try {
      ...
      // Create a blob from the data URL for better compatibility
      fetch(dataUrl)
        .then((res) => res.blob())
        .then((blob) => {
      ...
      // Fallback method
      try {
        const simpleDataUrl = canvasRef.current.toDataURL()
        window.open(simpleDataUrl)
      } catch (fallbackError) {
        console.error("Fallback error:", fallbackError)
        alert("Unable to download image. Please try a different browser or device.")
      }
    }
  }
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex flex-wrap gap-2 justify-center mb-4">
        ...
        {(showEmojiStamper || emojiStamperEnabled) && (
          <button
            onClick={() => {
              setTool("stamp")
